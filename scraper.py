...
def scraping(productcode):
    ...
    url = f"https://www.ceneo.pl/{productcode}#tab=reviews"
    ...
    while url:
        ...
        if response.status_code == requests.codes.ok:
            ...
            if len(opinions) > 0:
                ...
                print(f"There are no opinions about product with code {productcode}")
                url = None
                
    with open(f"app/static/opinions/{productcode}.json", "w", encoding="UTF-8") as jf:
